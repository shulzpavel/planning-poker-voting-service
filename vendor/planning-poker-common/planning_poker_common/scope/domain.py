...
from planning_poker_common.jira.role_contributors import person_bucket_key
...
def _issue_requires_jira_role(
    ...
    *,
    ...
    from planning_poker_common.jira.role_contributors import required_engineering_roles
...
_RELEASE_READY_TO_STORE_STATUS = frozenset({"к релизу", "ready for release", "to release", "ready to release", "release ready"})
_RELEASE_IN_TEST_STATUS_NAMES = frozenset({"тестирование", "к тестированию"})


def _is_ready_for_release_status(status: str) -> bool:
    normalized = (status or "").strip().casefold()
    if not normalized:
        return False
    if normalized in _RELEASE_READY_TO_STORE_STATUS:
        return True
    return (
        "к релиз" in normalized
        or "ready for release" in normalized
        or "ready to release" in normalized
        or normalized.endswith("to release")
    )


def classify_release_report_bucket(issue: dict[str, Any]) -> Literal["in_work", "in_test", "done", "open_questions", "not_started"]:
    """Bucket an issue for the mobile release scope report."""
    status, category = _status_tokens(issue)
    if _is_ready_for_release_status(status):
        return "done"
    if category == "done" or status in _DONE_STATUS_NAMES:
        return "done"
    if status == "пауза" or any(token in status for token in _PAUSE_STATUS_KEYWORDS):
        return "open_questions"
    if status in _RELEASE_IN_TEST_STATUS_NAMES:
        return "in_test"
    if category == "new" or status in _NOT_STARTED_STATUS_NAMES:
        return "not_started"
    return "in_work"
...
def release_done_subgroup(issue: dict[str, Any]) -> Literal["ready_for_release", "done"]:
    status, _ = _status_tokens(issue)
    if _is_ready_for_release_status(status):
        return "ready_for_release"
    return "done"


def sort_release_done_issues(issues: list[dict[str, Any]]) -> list[dict[str, Any]]:
    subgroup_rank = {"ready_for_release": 0, "done": 1}
    return sorted(
        issues,
        key=lambda issue: (
            subgroup_rank.get(release_done_subgroup(issue), 1),
...
def summarize_release_bucket(
    *,
    ...
    open_questions: list[dict[str, Any]] = []
    for issue in issues:
        bucket = classify_release_report_bucket(issue)
        if bucket == "done":
            done.append(issue)
        elif bucket == "in_test":
    ...
    return _apply_version_meta(
        ...
        "done": sort_release_done_issues(done),
